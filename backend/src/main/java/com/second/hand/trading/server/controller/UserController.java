...
//    }
    ...
    public ResultVo signIn(@RequestBody UserModel userModel) {
        ...
        // 检查手机号是否已存在
        if (userService.checkAccountNumberExists(userModel.getAccountNumber())) {
            return ResultVo.fail(ErrorMsg.ACCOUNT_EXIT,"手机号已存在");
        }

        // 检查昵称是否已存在
        if (userService.checkNicknameExists(userModel.getNickname())) {
            return ResultVo.fail(ErrorMsg.ACCOUNT_EXIT, "昵称已存在");
        }
